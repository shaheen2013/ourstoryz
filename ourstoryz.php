...
function register_ourstoryz_post_type() {
  $labels = array(
...
add_action('init', 'register_ourstoryz_post_type', 0);
global $wpdb;
...
if ($query->have_posts()) {
    ...
    while ($query->have_posts()) {
        ...
        echo '<td>' . esc_html($meta['event_type'][0]) . '</td>';
        echo '<td>' . esc_html($meta['organization_name'][0]) . '</td>';
        echo '<td>' . esc_html($meta['brand_logo'][0]) . '</td>';
        echo '<td>' . esc_html($meta['event_name'][0]) . '</td>';
        echo '<td>' . get_the_date() . '</td>';
        echo '<td><button class="view-button" data-record-id="' . esc_attr(get_the_ID()) . '">View</button></td>';
        echo '</tr>';
    }
